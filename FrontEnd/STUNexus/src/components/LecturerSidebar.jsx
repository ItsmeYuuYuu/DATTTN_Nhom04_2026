    ...
    <div className={`sidebar shadow-sm ${show ? 'show' : ''}`}>
      ...
      <div className="sidebar-nav">
        ...
        <NavLink to="/lecturer/subjects" onClick={onClose} className={({ isActive }) => `nav-item-link ${isActive ? 'active' : ''}`}>
          <FaBook />
          <span>Quản lý Môn & Lớp học Toàn khoa</span>
        </NavLink>
        <NavLink to="/lecturer/classes" onClick={onClose} className={({ isActive }) => `nav-item-link ${isActive ? 'active' : ''}`}>
          <FaBook />
          <span>Quản lý Lớp đang giảng dạy</span>
        </NavLink>
