...
namespace DiemDanhLopHoc.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> Login([FromBody] LoginRequestDto request)
        {
            ...
            if (admin != null)
            {
                var token = GenerateJwtToken(admin.TaiKhoan, "Admin", admin.HoTen, admin.MaQtv); // Thay 'Ma' bằng cột ID thật của bảng
                return Ok(new { success = true, message = "Đăng nhập thành công", data = new { token = token, role = "Admin" } });
            }
            ...
            if (giangVien != null)
            {
                var token = GenerateJwtToken(giangVien.TaiKhoan, "Lecturer", giangVien.HoTen, giangVien.MaGv);
                return Ok(new { success = true, message = "Đăng nhập thành công", data = new { token = token, role = "Lecturer" } });
            }
            ...
            if (sinhVien != null)
            {
                var token = GenerateJwtToken(sinhVien.TaiKhoan, "Student", sinhVien.HoTen, sinhVien.MaSv);
                return Ok(new { success = true, message = "Đăng nhập thành công", data = new { token = token, role = "Student" } });
            }
